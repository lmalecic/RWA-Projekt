...
namespace WebAPI.Controllers
{
    ...
    public class UserReservationController : ControllerBase
    {
        ...
        // PUT: api/UserReservation
        [HttpPut]
        public IActionResult Put([FromBody] UserReservationUpdateDto updateDto)
        {
            try {
                var dbEntity = _mapper.Map<UserReservation>(updateDto);
                var updatedEntity = _reservationService.Update(dbEntity);
                var mapped = _mapper.Map<UserReservationDto>(updatedEntity);
                return Ok(mapped);
            }
            catch (FileNotFoundException ex) {
                return NotFound(ex.Message);
